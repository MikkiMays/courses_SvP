...
public class MONTAGEnetworkController {
    ...
    @FXML
    private Button backNetwork;

    @FXML
    private Button signUpMontage;
    ...
    void initialize() {
        ...
        backNetwork.setOnAction(event -> {
            backNetwork.getScene().getWindow().hide();
            openScene("/svp/NETWORKS.fxml");
        });
        signUpMontage.setOnAction(event -> {
            DatabaseHandler dbHandler = new DatabaseHandler();
            ResultSet result = dbHandler.checkCourse(Const.COURSE_NETWORKMONTAGE, SvP.id);
            try {
                if (result.next())
                    this.check = result.getInt(Const.COURSE_NETWORKMONTAGE);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (check == 0){
                try {
                    dbHandler.signUpCourse(Const.COURSE_NETWORKMONTAGE, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        unsubscribe.setOnAction(event -> {
            ...
            try {
                if (result.next())
                    checkunsub = result.getInt(Const.COURSE_NETWORKMONTAGE);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (checkunsub == 1){
                try {
                    dbHandler.unSubCourse(Const.COURSE_NETWORKMONTAGE, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
