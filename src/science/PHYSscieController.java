...
public class PHYSscieController {
    ...
    @FXML
    private Button backScience;

    @FXML
    private Button signUpPhys;
    ...
    void initialize() {
        ...
        backScience.setOnAction(event -> {
            backScience.getScene().getWindow().hide();
            openScene("/svp/SCIENCE.fxml");
        });
        signUpPhys.setOnAction(event -> {
            DatabaseHandler dbHandler = new DatabaseHandler();
            ResultSet result = dbHandler.checkCourse(Const.COURSE_SCIEPHYS, SvP.id);
            try {
                if (result.next())
                    this.check = result.getInt(Const.COURSE_SCIEPHYS);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (check == 0){
                try {
                    dbHandler.signUpCourse(Const.COURSE_SCIEPHYS, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        unsubscribe.setOnAction(event -> {
            ...
            try {
                if (result.next())
                    checkunsub = result.getInt(Const.COURSE_SCIEPHYS);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (checkunsub == 1){
                try {
                    dbHandler.unSubCourse(Const.COURSE_SCIEPHYS, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
