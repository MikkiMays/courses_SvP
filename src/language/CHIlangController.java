...
public class CHIlangController {
    ...
    @FXML
    private Button backLang;

    @FXML
    private Button signUpChinaLang;
    ...
    void initialize() {
        ...
        backLang.setOnAction(event -> {
            backLang.getScene().getWindow().hide();
            openScene("/svp/LANGUAGE.fxml");
        });
        signUpChinaLang.setOnAction(event -> {
            DatabaseHandler dbHandler = new DatabaseHandler();
            ResultSet result = dbHandler.checkCourse(Const.COURSE_LANGCHI, SvP.id);
            try {
                if (result.next())
                    this.check = result.getInt(Const.COURSE_LANGCHI);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (check == 0){
                try {
                    dbHandler.signUpCourse(Const.COURSE_LANGCHI, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        unsubscribe.setOnAction(event -> {
            ...
            try {
                if (result.next())
                    checkunsub = result.getInt(Const.COURSE_LANGCHI);
                    } catch (SQLException ex) {
                Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (checkunsub == 1){
                try {
                    dbHandler.unSubCourse(Const.COURSE_LANGCHI, SvP.id);
                } catch (SQLException ex) {
                    Logger.getLogger(GRAPHdesignController.class.getName()).log(Level.SEVERE, null, ex);
                }
